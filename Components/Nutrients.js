  ...
  return (
    <View style={styles.nutrientGraph}>
      <View style={styles.nutrientNameContainer}>
        <Text style={[styles.nutrientName, { color: "black" }]}>
          {firstWordOfNutrientName}
        </Text>
...
const NutrientsComponent = ({ userData }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        fetch(`http://184.72.73.229:5001/get_user_data?email=${email}`)
  ...
  const nutrientsIntake =
    nutrients_intake.length > 0 ? nutrients_intake[0][0] : {};
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.cardContent}>
          {mappedNutrients.map(
            ({ nutrientName, value, color, totalValue, key }) => (
              <NutrientGraph
                key={key}
